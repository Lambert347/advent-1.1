...
List<string> fullSeedList = getFullSeedList(seedInfo);

List<Dictionary<long, string>> mapList = createMapList(trimList(inputList));

foreach(string seed in fullSeedList)
{

    long runningLocator = 0;

    long baseSeedNumber = Convert.ToInt64(seed.Split(",").ToList()[0]);
    long iterationNumber = Convert.ToInt64(seed.Split(",").ToList()[1]);

    for(int i = 0; i < iterationNumber; i++)
    {
        foreach (var dict in mapList)
        {
            long seedToCheck = baseSeedNumber + i;
            if(mapList.IndexOf(dict) == 0)
            {
                runningLocator = findKeyInDict(dict, seedToCheck);
            }
            else
            {
        ...
        if(fullSeedList.IndexOf(seed) == 0)
        {
...
static List<string> getFullSeedList(List<string> seedList)
{
    List<string> fullList = new List<string>();

    string baseSeedNumber = " ";

    for(int i = 0; i < seedList.Count; i++)
    {
        if(i == 0 || i % 2 == 0)
        {
            baseSeedNumber = seedList[i];
        }
        else 
        {
            fullList.Add(baseSeedNumber + "," + seedList[i]);
        }
    }
    return fullList;
}